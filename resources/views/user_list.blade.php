...
<body>
                    ...
                    @foreach ($users as $index => $item)
                        <tr data-bs-toggle="modal" data-bs-target="#infoModal{{ $item->id }}" class="clickable-row">
                            ...
                            <td class="edit-column no-print">
                                <button class="btn btn-info" style="height:39px"
                                    onclick="confirmPassword('update', {{ $item->id }})">Update</button>
                            </td>
                            <td class="delete-column no-print">
                                <form id="deleteForm{{ $item->id }}" action="{{ route('user.destroy', $item->id) }}"
                                    method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-danger"
                                        onclick="confirmPassword('delete', {{ $item->id }})">Delete</button>
                                </form>
                            </td>
                        </tr>
                            ...
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="infoModalLabel{{ $item->id }}">{{ $item->brand }} -
                                            {{ $item->fabric }}</h5>
    ...
    <script>
         ...
         function promptPassword() {
            ...
            if (password === "admin00") {
                window.location.href = "/form"; // Redirect to the Add Fabric page
            } else {
